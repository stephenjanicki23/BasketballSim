...
2. **Turnover check** — lineup ball-handling and IQ against the defence's
   hands, adjusted for pressure tactics, chemistry and fatigue. Steals get
   attributed to a specific defender.
3. **Non-shooting foul** — charged to a defender, and if the team is in the
   bonus it becomes two free throws.
4. **Shot selection** — a shooter is picked by usage tendency and current
   condition, then a zone (rim / paint / mid-range / corner three / above the
   break) from his tendencies, the offensive scheme, and how the defence is
   protecting the rim and closing out.
5. **Resolution** — make probability is the league baseline for that zone plus
   the shooter's edge over his primary defender and the team defence behind
   him, plus chemistry, spacing and fatigue. Blocks, shooting fouls and
   and-ones branch out of here.
6. **Assist** — credited on makes, more often on threes, in motion offences,
   and with better chemistry.
7. **Rebound** — offensive rebound chance from the two lineups' glass ratings
   and crash tactics; an offensive board triggers a short-clock second-chance
   possession.
...
|---|---|---|
| Points | 108.6 | 114 |
| Possessions | 101.3 | 99 |
| FG% / 3P% / FT% | .428 / .375 / .777 | .472 / .366 / .783 |
| AST / TOV / REB | 22.9 / 13.9 / 54.7 | 26.5 / 13.5 / 53 |
| STL / BLK / PF | 8.6 / 6.1 / 16.8 | 7.5 / 5.0 / 19 |
| Score SD / mean margin | 15.8 / 15.6 | ~13 / ~11.5 |
...
One known gap: **games are more spread out than real ones** — mean margin 15.6
...
```
bballsim/
  ratings.py       attributes + the 0-99 scale, normalized for the engine
  ...
  chemistry.py     team / pair / lineup-fit chemistry -> an execution modifier
